...
const menuGroups = [
  // ─── กลุ่ม 2: เมนูปลาร้าทั้งหมด ───
  {
    ...
    items: [
      ...
      simpleItem("tam-plara-squid-neck", "ตำปลาร้าคอหมึก", 89, "คอหมึกเด้งกรุบๆ ในตำปลาร้ารสแซ่บ", true),
    ],
  },
  ...
  // ─── กลุ่ม 4: เมนูยำแซ่บ ───
  {
    ...
    items: [
      ...
      simpleItem("larb-snail", "ลาบหอย", 49, "หนุบหนับ รสแซ่บ เคี้ยวเพลิน", true),
      simpleItem("larb-minced-pork", "ลาบหมูสับ", 49, "หมูสับจุกๆ แซ่บไม่เหมือนใคร", true),
      simpleItem("namtok-pork", "น้ำตกหมูย่าง", 49, "หมูย่างชิ้นโตๆ ราดน้ำตก รสเข้มข้น", true),
      simpleItem("yum-pickled-crab", "ยำปูเปรี้ยว", 69, "ปูเปรี้ยวสดๆ ดองเอง รสแซ่บ", true),
      simpleItem("larb-pickled-crab", "ลาบปูเปรี้ยว", 69, "หอมข้าวคั่ว ปูเปรี้ยวสดๆ รสแซ่บ", true),
    ],
  },
  // ─── กลุ่ม 5: ของกินคู่กัน ───
  {
    ...
    items: [
      simpleItem("grilled-chicken", "ไก่ย่าง", 20, "ชุด3ไม้ รอสุก10นาที<br>ย่างใหม่ ทุกครั้ง", false),
      simpleItem("sticky-rice", "ข้าวเหนียว", 10, "นึ่งใหม่ๆร้อนๆ", false),
      simpleItem("grilled-pork", "หมูย่าง", 49, "อบใหม่ รอสุก20นาที<br>สูตรหมัก ไม่ซ้ำใคร", false),
      simpleItem("kanomjeen", "ขนมจีน", 10, "ขนมจีนเส้นนุ่ม", false),
      simpleItem("pork-crackling", "แคปหมู", 10, "กรอบๆ อร่อยฟิน", false),
    ],
  },
  // ─── กลุ่ม 6: เเครื่องดื่มแนะนำ ───
  {
    ...
    items: [
      ...
      simpleItem("grass-jelly-milk", "เฉาก๊วยนมสด", 35, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("grass-jelly-classic", "เฉาก๊วยโบราณ", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("fresh-coconut", "น้ำมะพร้าวสด", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("water", "น้ำเปล่า", 10, "น้ำดื่มขวดละ 10 บาท", false),
      variantItem(
        "soft-drink",
        "น้ำอัดลม",
        "เลือกขนาด",
  ...
  // ─── กลุ่ม 7: ครื่องดื่ม ชา/กาแฟ ───
  {
    ...
    items: [
      ...
      simpleItem("black-coffee", "กาแฟดำ", 35, "กดเพิ่มลงตะกร้าได้ทันที", false),
  ...
  // ─── กลุ่ม 8: เครื่องดื่ม โซดา ───
  {
    ...
    items: [
      simpleItem("red-soda", "น้ำแดงโซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("red-lime-soda", "น้ำแดงโซดามะนาว", 35, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("lychee-soda", "ลิ้นจี่โซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("apple-soda", "แอปเปิ้ลโซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("grape-soda", "องุ่นโซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("blue-hawaii", "บลูฮาวายโซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("strawberry-soda", "สตอเบอรี่โซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("orange-soda", "ส้มโซดา", 30, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("honey-lime", "น้ำผึ้งมะนาว", 35, "กดเพิ่มลงตะกร้าได้ทันที", false),
      simpleItem("honey-lime-soda", "น้ำผึ้งมะนาวโซดา", 40, "กดเพิ่มลงตะกร้าได้ทันที", false),
    ],
  },
];
...
function simpleItem(id, name, price, description = "กดเพิ่มลงตะกร้าได้ทันที", needsSpice = false) {
  return {
    id,
    name,
    type: "simple",
...
function renderModalOptions(item) {
  ...
  if (item.type === "variant") {
    parts.push(`
      <label class="field-label">
        <span class="field-label__hint">👇 กินแบบไหนดีวันนี้ เลือกก่อนเพิ่มลงตะกร้า</span>
  ...
  if (item.needsSpice) {
    parts.push(`
      <label class="field-label">
        <span class="field-label__hint">👇 ระดับความเผ็ด เลือกก่อนเพิ่มลงตะกร้า</span>
...
function confirmAddFromModal() {
  ...
  document.querySelectorAll(`[data-add="${item.id}"]`).forEach((button) => {
...
function renderCart() {
  ...
  if (cartSummaryButton) cartSummaryButton.hidden = count === 0;
...
if (menuSearchInput) {
  ...
  if (initialSearch) {
    menuSearchInput.value = initialSearch;
    renderSearchResults(initialSearch);
  }
}
